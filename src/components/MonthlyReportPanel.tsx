        ...
        <div key={d.monthKey} className="flex flex-1 flex-col items-center gap-0.5">
          <div className="flex w-full items-end gap-0.5" style={{ height: 90 }}>
            <div
              ...
              style={{ height: `${(d.created / max) * 90}px`, minHeight: d.created > 0 ? 2 : 0 }}
              title={`Criados: ${d.created}`}
            />
            <div
              className="flex-1 rounded-t-sm bg-emerald-500 opacity-80"
              style={{ height: `${(d.finalized / max) * 90}px`, minHeight: d.finalized > 0 ? 2 : 0 }}
        ...
        <div className="space-y-5">
          ...
          {charts.includes("summary") && (
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: "Clientes criados", value: data.summary.totalCreated, color: "text-blue-500", border: "border-blue-300 dark:border-blue-800" },
                { label: "Finalizados", value: data.summary.totalFinalized, color: "text-emerald-600 dark:text-emerald-400", border: "border-emerald-300 dark:border-emerald-800" },
                { label: "Localizados", value: data.summary.totalLocalized, color: "text-amber-600 dark:text-amber-400", border: "border-amber-300 dark:border-amber-800" },
              ].map((c) => (
              ...
              <div className="space-y-2">
                {data.byCollaborator.map((c) => (
                  <div key={c.name} className="space-y-1">
                    <HorizontalBar label={`${c.name} (criados)`} value={c.created} max={data.byCollaborator[0]?.created ?? 1} color="#3b82f6" />
                    {c.finalized > 0 && (
                      <HorizontalBar label={`${c.name} (fin.)`} value={c.finalized} max={data.byCollaborator[0]?.created ?? 1} color="#22c55e" />
                    )}
                  </div>
                ))}
              </div>
