...
export function SelfServicePonto({ user }: { user: SessionUser }) {
  ...
  const verifiedDescriptorRef = useRef<number[] | null>(null);
  ...
  useEffect(() => {
    void loadFaceModels().then(() => setModelsLoaded(true)).catch(() => {});
  }, []);
  ...
  function stopClockCamera() {
    ...
    verifiedDescriptorRef.current = null;
    setClockMsg("");
  ...
  const detectFace = useCallback(async () => {
    ...
    try {
      ...
      if (tick.status === "ready" && det) {
        if (intervalRef.current) { clearInterval(intervalRef.current); intervalRef.current = null; }
        streamRef.current?.getTracks().forEach((t) => t.stop());
        streamRef.current = null;
        const finalConfidence = tick.averageConfidence;
        verifiedDescriptorRef.current = Array.from(det.descriptor);
  ...
  const submitPunch = useCallback(async (type: PontoType) => {
    if (verifiedConfidence === null || !verifiedDescriptorRef.current) return;
    ...
    try {
      ...
      const res = await fetch("/api/ponto", {
        ...
        body: JSON.stringify({
          ...
          teamId: user.teamId,
          type,
          descriptor: verifiedDescriptorRef.current,
          origin: "MOBILE",
      ...
      setClockMsg(`${pontoTypeLabel(type)} registrada às ${fmtPontoTime(new Date().toISOString())}`);
          ...
          <div className="divide-y divide-border/50">
            {records.map((r) => (
              <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <span className="text-sm font-medium text-foreground tabular-nums">{fmtPontoTime(r.recordedAt)}</span>
              </div>
            ))}
          </div>
        )}
